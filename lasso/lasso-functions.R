LASSO <- function(ID){
  simData.gene.learn <- simData.learn[,-c(which(colnames(simData.learn) == ID))]
  simData.gene.test <- simData.test[,-c(which(colnames(simData.test) == ID))]
  gene.learn <- simData.learn[,which(colnames(simData.learn) == ID)]
  gene.test <- simData.test[,which(colnames(simData.test) == ID)]
